...
UPDATE User u
INNER JOIN tiktok_creator_score.users tcs_users ON u.email = tcs_users.username
SET
  u.kindleScore = tcs_users.account_total_score
WHERE
  tcs_users.account_total_score IS NOT NULL;

SET
  updated_users = ROW_COUNT ();
...
FROM
  User u
  INNER JOIN tiktok_creator_score.users tcs_users ON u.email = tcs_users.username
  INNER JOIN tiktok_creator_score.keyword_scores ks ON tcs_users.id = ks.user_id
    ...
    (
      ...
      FROM
        JSON_TABLE (
          ...
          CONCAT ('["', REPLACE (ks.keyword, ' | ', '","'), '"]'),
          '$[*]' COLUMNS (item VARCHAR(255) PATH '$')
        ) AS jt2
    )
  )
  LEFT JOIN CampaignUser existing_cu ON existing_cu.userId = u.id
...
INNER JOIN tiktok_creator_score.users tcs_users ON u.email = tcs_users.username
INNER JOIN tiktok_creator_score.keyword_scores ks ON tcs_users.id = ks.user_id
INNER JOIN Campaign c ON cu.campaignId = c.id
SET
...
SET
  cu.bonusScore = (
    SELECT
      COALESCE(SUM(FLOOR(cu_invitee.baseScore * 0.1)), 0)
    FROM
      CampaignUser cu_invitee
      INNER JOIN User u_invitee ON cu_invitee.userId = u_invitee.id
    WHERE
      u_invitee.inviterId = u.id
