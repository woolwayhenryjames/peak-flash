...
async function create100JENTestUsers() {
  console.log('🌱 开始批量创建100个JENtest用户...\n');
  ...
  try {
    ...
    // 创建100个用户（JENtest86 到 JENtest185）
    for (let i = 86; i <= 185; i++) {
      try {
        const userName = `JENtest${i.toString().padStart(3, '0')}`;
        const newUserId = randomUUID();
        const email = `jentest${i.toString().padStart(3, '0')}_${randomString(6)}@test.com`;
        ...
        errorCount++;
        console.error(`  ✗ 创建用户失败: JENtest${i.toString().padStart(3, '0')} - ${error.message}`);
      }
    }
