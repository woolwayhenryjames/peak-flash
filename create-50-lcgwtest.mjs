...
async function create50LCGWTestUsers() {
  console.log('🌱 开始创建50个LCGWtest用户...\n');
  ...
  try {
    ...
    // 创建50个用户（LCGWtest01 到 LCGWtest50）
    for (let i = 1; i <= 50; i++) {
      try {
        const userName = `LCGWtest${i.toString().padStart(2, '0')}`;
        const newUserId = randomUUID();
        const email = `lcgwtest${i.toString().padStart(2, '0')}_${randomString(6)}@test.com`;
        ...
        errorCount++;
        console.error(`  ✗ 创建用户失败: LCGWtest${i.toString().padStart(2, '0')} - ${error.message}`);
      }
    }
