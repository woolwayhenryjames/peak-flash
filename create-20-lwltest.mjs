...
async function create20LWLTestUsers() {
  console.log('🌱 开始创建20个LWLtest用户...\n');
  ...
  try {
    ...
    // 创建20个用户（LWLtest147 到 LWLtest166）
    for (let i = 147; i <= 166; i++) {
      try {
        const userName = `LWLtest${i.toString().padStart(3, '0')}`;
        const newUserId = randomUUID();
        const email = `lwltest${i.toString().padStart(3, '0')}_${randomString(6)}@test.com`;
        ...
        errorCount++;
        console.error(`  ✗ 创建用户失败: LWLtest${i.toString().padStart(3, '0')} - ${error.message}`);
      }
    }
    ...
    console.log(`  邀请人ID: qm2biJuxTwy7EIU3SBTI0AyvrzADr4Wx`);
