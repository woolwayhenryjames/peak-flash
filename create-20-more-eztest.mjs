...
async function create20MoreEZTestUsers() {
  console.log('🌱 开始创建更多20个EZtest用户...\n');

  try {
    // 首先验证邀请人是否存在
    const [inviter] = await connection.query(
      'SELECT * FROM User WHERE id = ?',
      ['ezLlWouWRz96x8rI1FkcE0Tc1ZWgGEQH']
    );
    ...
    // 创建20个用户（EZtest31 到 EZtest50）
    for (let i = 31; i <= 50; i++) {
      try {
        const userName = `EZtest${i.toString().padStart(2, '0')}`;
        const newUserId = randomUUID();
        const email = `eztest${i.toString().padStart(2, '0')}_${randomString(6)}@test.com`;
        ...
        await connection.query(
          ...
          [
            ...
            'ezLlWouWRz96x8rI1FkcE0Tc1ZWgGEQH', // inviterId
            0, // isBusiness = false
            0, // isAdmin = false
        ...
        if (successCount % 10 === 0) {
          console.log(`  ✓ [${successCount}/20] 已创建 ${successCount} 个用户`);
        }
      } catch (error) {
        errorCount++;
        console.error(`  ✗ 创建用户失败: EZtest${i.toString().padStart(2, '0')} - ${error.message}`);
      }
    }
    ...
    console.log(`  邀请人ID: ezLlWouWRz96x8rI1FkcE0Tc1ZWgGEQH`);
    ...
    // 统计EZtest用户总数
    const [ezCount] = await connection.query(
      "SELECT COUNT(*) as count FROM User WHERE name LIKE 'EZtest%'"
    );
    console.log(`\n📈 EZtest用户总数: ${ezCount[0].count} 个`);
