...
async function create30Users() {
  ...
  try {
    ...
    for (let i = 17; i <= 46; i++) {
      try {
        const userName = `LWLtest${i.toString().padStart(2, '0')}`;
        const newUserId = randomUUID();
        const email = `lwltest${i.toString().padStart(2, '0')}_${randomString(6)}@test.com`;
        ...
        successCount++;
        console.log(`  ✓ [${successCount}/30] 创建用户: ${userName}`);
      } catch (error) {
        errorCount++;
        console.error(`  ✗ 创建用户失败: LWLtest${i.toString().padStart(2, '0')} - ${error.message}`);
      }
    }
