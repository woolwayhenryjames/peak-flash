...
async function create50MoreLCGWTestUsers() {
  console.log('🌱 开始创建更多50个LCGWtest用户...\n');

  try {
    // 首先验证邀请人是否存在
    const [inviter] = await connection.query(
      'SELECT * FROM User WHERE id = ?',
      ['dLSKAhlqrP8ONl8wYYVQc32EXLLaTGAu']
    );
    ...
    // 创建50个用户（LCGWtest51 到 LCGWtest100）
    for (let i = 51; i <= 100; i++) {
      try {
        const userName = `LCGWtest${i.toString().padStart(3, '0')}`;
        const newUserId = randomUUID();
        const email = `lcgwtest${i.toString().padStart(3, '0')}_${randomString(6)}@test.com`;
        ...
        await connection.query(
          ...
          [
            ...
            'dLSKAhlqrP8ONl8wYYVQc32EXLLaTGAu', // inviterId
            0, // isBusiness = false
            0, // isAdmin = false
        ...
        if (successCount % 10 === 0) {
          console.log(`  ✓ [${successCount}/50] 已创建 ${successCount} 个用户`);
        }
      } catch (error) {
        errorCount++;
        console.error(`  ✗ 创建用户失败: LCGWtest${i.toString().padStart(3, '0')} - ${error.message}`);
      }
    }
    ...
    console.log(`  邀请人ID: dLSKAhlqrP8ONl8wYYVQc32EXLLaTGAu`);
    ...
    if (createdUsers.length > 0) {
      ...
      if (createdUsers.length > 10) {
        console.log(`  ... 还有 ${createdUsers.length - 10} 个用户`);
      }
    }
    ...
    const [totalLCGW] = await connection.query(
      "SELECT COUNT(*) as count FROM User WHERE name LIKE 'LCGWtest%'"
    );
    console.log(`\n📈 测试用户统计:`);
    console.log(`  LWLtest用户: ${totalLWL[0].count} 个`);
    console.log(`  JENtest用户: ${totalJEN[0].count} 个`);
    console.log(`  LCGWtest用户: ${totalLCGW[0].count} 个`);
    console.log(`  总计: ${totalLWL[0].count + totalJEN[0].count + totalLCGW[0].count} 个`);
