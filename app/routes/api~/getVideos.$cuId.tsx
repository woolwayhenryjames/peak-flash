...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const parsedData: UserVideo[] = parsedVideoScores.map((video, index) => ({
      id: index, // Placeholder, as this field is required
      videoId: video.video_id,
      description: video.desc,
      createTime: new Date(video.create_time),
      ...
      collectCount: video.statistics.collect_count,
      aiScore: video.ai_quality_score.total_score,
      aiScoreDetails: parsedAiScores[video.video_id] || null,
      videoUrl: parsedAiScores[video.video_id]?.media_urls?.video_url || null,
      coverUrl: parsedAiScores[video.video_id]?.media_urls?.cover_url || null,
      dynamicCoverUrl:
        parsedAiScores[video.video_id]?.media_urls?.dynamic_cover_url || null,
      keywordScore: video.ai_quality_score.keyword_score,
      originalityScore: video.ai_quality_score.originality_score,
      clarityScore: video.ai_quality_score.clarity_score,
      spamScore: video.ai_quality_score.spam_score,
      promotionScore: video.ai_quality_score.promotion_score,
      engagementRates: video.engagement_rates,
      totalScore: video.ai_quality_score.total_score,
      campaignUserId: 0,
      // Default values for fields not present in the JSON
      active: true,
    }));
