...
async function create50Jen2TestUsers() {
  console.log('🌱 开始创建50个jen2test用户...\n');
  ...
  try {
    ...
    // 创建50个用户（jen2test01 到 jen2test50）
    for (let i = 1; i <= 50; i++) {
      try {
        const userName = `jen2test${i.toString().padStart(2, '0')}`;
        const newUserId = randomUUID();
        const email = `jen2test${i.toString().padStart(2, '0')}_${randomString(6)}@test.com`;
        ...
        errorCount++;
        console.error(`  ✗ 创建用户失败: jen2test${i.toString().padStart(2, '0')} - ${error.message}`);
      }
    }
